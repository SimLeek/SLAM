...
if __name__ == '__main__':
    ...
    def simu():
        ...
        nbLandmark = 900
        #nbLandmark = 42
        delta_t = 0.1   #time elapsed between measurements
        maxSpeed = 5
        ...
        with open("data/map_data.txt") as M:
            mapShape = []
            for line in M:
                line = line.split()
                if line:
                    line = [float(i) for i in line]
                    mapShape.append(line)
        map = np.asarray(mapShape)

        with open("data/control_data.txt") as C:
            controlShape = []
            for line in C:
                line = line.split()
                if line:
                    line = [float(i) for i in line]
                    controlShape.append(line)
        control = np.asarray(controlShape)
